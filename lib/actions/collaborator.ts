...
const assertRepoInInstallation = async (
  ...
  if (!isInstalledForRepo) throw new Error(`"${owner}/${repo}" is not part of this editor's installation.`);
    ...
    for (const email of emails) {
      ...
      if (!existingUser) {
        ...
        try {
          ...
          await sendEmail({
            to: normalizedEmail,
            subject: `You've been invited to edit ${repo}`,
            html,
          });
        } catch (error: any) {
